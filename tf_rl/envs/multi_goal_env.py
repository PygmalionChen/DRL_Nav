...
class Listen_class():
    ...
    def scan_callback(self, msg, args):     # 回调函数的逻辑？返回值是什么？
...
class GazeboMultiGoalEnv8(gazebo_env.GazeboEnv):
    def __init__(self):
        ...
        self.nor = 8  # The nor means the number of robots
        self.init_pose = []
    ...
    def reward_fun(self, dis):
        ...
        while(dis.size>1):
            ...
            dis = np.delete(dis, r, 0) # Remove the r-th member along the 0-axis of the object dis.
    ...
    def _step(self, action, goal, first=False):
        ...
        for i in range(self.nor):
            ...
            if goal_dis < 0.4/self.max_range_dis:
                done_list[i] = True
                reward_list[i] += 1
                # self.resetState(i)
            theta = math.atan2(2 * self.listen_class.odom_list[i].pose.pose.orientation.z *
                    ...
                    math.atan2(goal[i][1] - self.listen_class.odom_list[i].pose.pose.position.y,
                               goal[i][0] - self.listen_class.odom_list[i].pose.pose.position.x)
        ...
        # 返回一次交互数据
        return np.array(state_list), np.array(param_list), reward_list, done_list  # , self.pong_list
